...
Layers mode keeps ingredients apart for explanation. Stirred mode runs a 3.2-second spoon-and-swirl animation, delays colour blending until the initial stirring motion is visible, and ultimately replaces the stack with one cylinder. Preparation automatically switches to Stirred after the final ingredient. The jug contains ingredient-coloured liquid with a sloped, rippling surface that drains deterministically as the timeline advances, including when scrubbing backward. This is educational animation, not computational fluid dynamics. Colours, cube sizes, ice displacement, sugar quantities and volumes are illustrative. They are not nutrition measurements or physical simulation outputs.
